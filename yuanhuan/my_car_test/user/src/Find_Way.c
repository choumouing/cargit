...
// 
		for(int i = SEARCH_IMAGE_H-20;i>20;i--)
	  {
		...
		for(int i = SEARCH_IMAGE_H-1;i>SEARCH_IMAGE_H - 16;i--)
		{
		...
		start_point = find_jump_point(right_edge_line,SEARCH_IMAGE_H - 40,5,5,0);
		if(start_point)
			{
			island_temp_flag = 40;
			end_point = image_find_circle_point(right_edge_line,SEARCH_IMAGE_H - 10,30);
			}
		if(end_point)
		{
				island_temp_flag = 50;
...
void FindIsland_Out()
{
	if(island_temp_flag == 1 && encoder_temp > 1100)
	{
		center_line_mode = 1;
		angle_temp = 0;
		encoder_temp = 0;
	...
	if(island_temp_flag == 2 && angle_temp > 340)
	{
		center_line_mode = 0;
		angle_temp = 0;
		encoder_temp = 0;
